...
file_name_names <- "/workspace/corise-r-for-ds-practice/data/names.csv.gz"
...
tbl_lifetables |> 
  ...
  labs(
    title = " Graph of population alive between the years 1900 and 2022 ",
    subtitle = "Population of females alive exceeds male",
    x = " Years",
...
tbl_lifetables_extended_2022 |> 
 ...
 geom_line() +
  ...
  # Add labels (title, subtitle, x, y, caption)
labs(
    title = "Interpolated Distribution Of Population Alive Between The Years 1900 and 2022 ",
    x = " Years",
    y = " Probability Of Being Alive",
    caption = "Source: FiveThirtyEight"
  ) +
...
#filter (name == "Mary", between(year,1950,1960))
tbl_names_extended
...
geom_col(aes(y = nb_alive, fill = "white",color = "yellow")) + 
    # Add a line layer with y = nb_births, and appropriate colors
 geom_line(aes(y = nb_births, color = "grey")) + 
...
ggplot(aes(x = age_median,y = name)) +
  ...
  labs(
    title = "Median Age for Females With the 25 Most Common Names",
